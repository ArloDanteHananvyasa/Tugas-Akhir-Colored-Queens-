...
\begin{document}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}{0.35\textwidth}
			\centering
			\includegraphics[width=\linewidth]{n+kSolution.jpg}
			\caption*{Gambar 2(a) Contoh solusi salah masalah \textit{8+1 Queens}}
			\footnotesize Sumber: \url{http://www.npluskqueens.info/background.html}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			Secara konseptual, permasalahan ini dapat direpresentasikan sebagai \textit{Constraint Satisfaction Problem} (CSP), yang terdiri dari tiga komponen utama: variabel, domain, dan constraint. Setiap variabel mewakili baris pada papan, dengan domain berupa pilihan kolom tempat menteri dapat ditempatkan. Constraint pada N-Queens memastikan bahwa tidak ada dua variabel yang bernilai sama (menghindari konflik kolom), serta memastikan bahwa perbedaan nilai antar variabel tidak sama dengan selisih indeks barisnya (menghindari konflik diagonal). Formulasi CSP ini memberikan sejumlah keuntungan, terutama karena memungkinkan algoritma pencarian untuk memanfaatkan teknik penyempitan domain seperti \textit{forward checking} atau propagasi kendala seperti AC-3.
			...
			Studi literatur menunjukkan bahwa permasalahan N-Queens memiliki banyak varian yang dikembangkan untuk mengeksplorasi sifat-sifat baru dari struktur constraint maupun untuk menguji kemampuan algoritma pencarian dalam kondisi yang lebih menantang. Dua varian yang cukup menonjol dalam penelitian teoretis adalah \textit{N+k Queens} dan \textit{Toroidal Queens}. Keduanya memodifikasi aturan dasar masalah sehingga menghasilkan ruang pencarian yang berbeda secara signifikan dari N-Queens klasik.
			
			\textbf{1. Variansi \textit{N + k Queens}} \\
			Pada varian N+k Queens, jumlah bidak yang harus ditempatkan tidak lagi sama dengan ukuran papan. Jika pada permasalahan standar setiap baris harus diisi tepat satu menteri, pada varian ini digunakan papan berukuran $n \times n$ namun dengan total bidak sebanyak $n + k$, di mana $k$ dapat bernilai positif atau negatif. Ketika $k > 0$, tujuan dari masalah menjadi mencari konfigurasi di mana lebih dari $n$ menteri ditempatkan tanpa adanya konflik horizontal, vertikal, maupun diagonal. Sebaliknya, ketika $k < 0$, jumlah menteri lebih sedikit dari ukuran papan, dan fokusnya beralih dari penempatan wajib per baris menjadi pemilihan subset baris mana saja yang akan diisi.
			
			Variasi ini tidak hanya memperluas ruang solusi tetapi juga mengubah struktur constraint. Pada N-Queens klasik, setiap baris memiliki tepat satu variabel, namun pada N+k Queens jumlah variabel dan struktur domainnya menjadi lebih fleksibel. Permasalahan ini lebih mirip \textit{extended CSP}, di mana pemilihan variabel aktif menjadi bagian dari proses pencarian. Literatur menunjukkan bahwa varian N+k Queens menarik karena beberapa nilai $k$ memungkinkan konfigurasi solusi yang tidak mungkin terjadi pada N-Queens standar. Misalnya, untuk $k$ tertentu, solusi yang memanfaatkan simetri papan atau pola diagonal alternatif dapat muncul, sehingga analisis struktur solusinya jauh lebih beragam. Permasalahan ini sering digunakan untuk menguji performa algoritma heuristik atau metaheuristik, sebab pertambahan jumlah menteri memperbesar dan memadatkan ruang konflik, meningkatkan kebutuhan strategi penyempitan domain yang efektif.
			
			\textbf{2. Variansi \textit{Toroidal Queens}} \\
			Varian toroidal merupakan modifikasi signifikan dari permasalahan N-Queens yang mengubah topologi papan dari bidang datar menjadi permukaan torus. Dalam papan toroidal, tepi kanan terhubung dengan tepi kiri, dan tepi atas terhubung dengan tepi bawah, sehingga gerakan menteri bersifat \textit{wrap-around}. Ini berarti bahwa serangan diagonal, vertikal, dan horizontal tidak berhenti pada batas papan, melainkan berlanjut melewati tepi dan kembali dari sisi yang berlawanan.
			
			Perubahan topologi ini memodifikasi definisi konflik secara drastis. Sebagai contoh, dua menteri yang ditempatkan pada kolom ujung kiri dan kolom ujung kanan dengan baris yang sama akan tetap saling menyerang secara horizontal karena papan membentuk lingkaran pada orientasi tersebut. Demikian pula, serangan diagonal tidak hanya mempertimbangkan selisih baris dan kolom, tetapi juga efek pembungkusan yang terjadi pada kedua sumbu. Studi klasik (misalnya oleh Pólya) menunjukkan bahwa solusi pada papan toroidal sangat dibatasi untuk ukuran tertentu. Salah satu hasil paling terkenal adalah bahwa permasalahan toroidal N-Queens hanya memiliki solusi jika dan hanya jika $n$ tidak memiliki faktor pembagi $2$ atau $3$. Dengan kata lain, solusi hanya ada jika $n$ relatif prima terhadap $6$ (i.e., $\gcd(n, 6) = 1$). 
			
			Keterbatasan teoretis ini membuat varian toroidal jauh lebih ketat dibandingkan N-Queens klasik yang memiliki solusi untuk semua $n \geq 4$. Dalam konteks CSP, perubahan topologi ini mengubah bentuk constraint diagonal dan menghadirkan pola konflik yang bersifat siklik, yang sering kali menghasilkan grafik constraint yang jauh lebih padat. Untuk algoritma pencarian, toroidal queens memberikan tantangan unik karena banyak heuristik standar (misalnya pembatasan serangan diagonal berbasis selisih baris dan kolom) perlu disesuaikan agar mempertimbangkan pembungkusan tepi. Varian ini menjadi studi penting dalam bidang teori graf, teori grup, dan pengembangan algoritma karena struktur konflik yang lebih kaya dibandingkan permasalahan N-Queens tradisional.
			
			Secara keseluruhan, kedua varian ini memberikan wawasan penting mengenai bagaimana perubahan jumlah variabel dan perubahan topologi papan dapat mengubah sifat dasar constraint, ruang solusi, dan strategi algoritmik yang diperlukan untuk menyelesaikan masalah. Pemahaman terhadap varian-varian ini memperkaya perspektif terhadap fleksibilitas formulasi N-Queens dan menunjukkan bagaimana permasalahan tersebut dapat berkembang menjadi bentuk-bentuk yang lebih kompleks, yang pada akhirnya membantu dalam mempelajari varian lain seperti Colored Queens.
