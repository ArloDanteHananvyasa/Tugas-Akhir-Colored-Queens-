...
\begin{document}
	...
	Sebelum masuk ke pembahasan lebih lanjut, berikut disajikan rumusan masalah yang menjadi dasar penelitian ini:
	\begin{itemize}	
	...
	\section{Tujuan}
	Selanjutnya, tujuan dari penelitian ini dapat dirumuskan sebagai berikut:
	\begin{itemize}	
	...
	\textbf{Cara Kerja Backtracking (Step-by-Step) dan Implementasi pada N-Queens}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.3\linewidth]{psoflow.png}
	...
	\begin{itemize}
		...
		\textit{Metode utama:}
		\begin{itemize}
			\item \texttt{BacktrackingSolverAC3(board)}: Konstruktor yang menginisialisasi solver dengan board, membangun struktur data BitSet untuk tracking domain valid, dan menyiapkan stack untuk undo pruning
			\item \texttt{solve()}: \textit{Entry point} utama yang memanggil \texttt{placeQueens(0)} dan menghitung statistik waktu eksekusi
			\item \texttt{placeQueens(colorIndex)}: Metode rekursif untuk menempatkan menteri pada setiap warna, mencoba setiap posisi valid dan melakukan \textit{backtrack} jika gagal
			\item \texttt{propagateConstraints(row, col, placedColorIndex)}: Melakukan propagasi kendala AC-3 dengan dua tahap: (1) \textit{forward checking} untuk menghapus nilai yang konflik dengan posisi yang baru ditempatkan, (2) propagasi berbasis \textit{queue} untuk memeriksa apakah penghapusan menyebabkan nilai lain kehilangan \textit{support}
			\item \texttt{hasAnySupport(cell, cellColorIdx, excludeColorIdx, placedColorIndex)}: Memeriksa apakah sebuah sel memiliki \textit{support} (nilai yang kompatibel) dari semua domain warna lainnya
			\item \texttt{conflicts(cell1, cell2)}: Memeriksa apakah dua sel berkonflik (berada di baris/kolom sama atau bersebelahan)
			\item \texttt{computeAttackZone(row, col)}: Menghitung semua posisi yang berkonflik dengan posisi \texttt{(row, col)}, termasuk seluruh baris, kolom, dan 8 sel bersebelahan
			\item \texttt{undoPrunes(pruneStartPos)}: Mengembalikan semua nilai yang dipangkas sejak posisi tertentu di stack dengan memulihkan BitSet
			\item \texttt{anyColorExhausted(currentColorIndex)}: Memeriksa apakah ada warna yang domainnya kosong setelah propagasi (\textit{domain wipeout})
			\item \texttt{getSolution()}: Mengembalikan array solusi (indeks posisi untuk setiap warna)
			\item \texttt{getSteps()}, \texttt{getBacktracks()}, \texttt{getExecutionTime()}: Mengembalikan statistik eksekusi algoritma
		\end{itemize}
	...
	\begin{enumerate}
		...
		Halaman \ref{fig:mockup_homepage} merupakan titik masuk pertama ketika pengguna mengunjungi situs \textit{Colored Queens}. Pengguna diperkenalkan dengan tampilan sederhana yang berfokus pada tombol utama untuk memulai permainan.
		...
		Pada halaman \ref{fig:mockup_game}, pengguna berinteraksi langsung dengan papan permainan \textit{Colored Queens}. Pengguna dapat menempatkan atau memindahkan bidak menteri untuk menyelesaikan \textit{puzzle}.
		
		\begin{figure}
			\centering
			\includegraphics[width=1\linewidth]{/mockup/level.png}
			\caption{Tampilan halaman permainan yang menampilkan papan \textit{Colored Queens} sesuai tingkat kesulitan yang dipilih.}
		...
		Ketika pemain melakukan langkah yang melanggar aturan \textit{Colored Queens}, sistem akan menampilkan peringatan visual. Peringatan ini membantu pemain memahami jenis pelanggaran (bersebelahan, horizontal, vertikal, atau warna sama). Contoh dari fitur ini dapat dilihat pada gambar \ref{fig:mockup_warning_adjacent}, \ref{fig:mockup_warning_horizontal}, \ref{fig:mockup_warning_vertical}, dan \ref{fig:mockup_warning_color}.
		...
		\begin{figure}
			...
			\caption{Tampilan halaman kemenangan setelah pemain menyelesaikan puzzle \textit{Colored Queens}.}
